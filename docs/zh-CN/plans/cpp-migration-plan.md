...
将原始的 Rust 版 IPMI 风扇控制工具迁移为 C++ 版本，引入 INI 阶梯控速，增加 `systemd` 服务安装能力，并用面向 C++ 的 GitHub Actions 替换旧的 Rust 自动化流程。

## 当前状态

1. 使用 `CMake + yaml-cpp` 重建工程
2. 用 INI 配置替代硬编码的温度映射
3. 保留 `help / fixed / auto`，并新增 `validate-config / install-service`
